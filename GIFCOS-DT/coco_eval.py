...
class COCOGenerator(CocoDetection):
    ...
    def __init__(self,imgs_path,anno_path,resize_size=[980,1600]):
...
def evaluate_coco(generator, model, threshold=0.25):
    ...
    # write output
    json.dump(results, open('6_tract_lesions.json', 'w'), indent=4)
    # json.dump(image_ids, open('{}_processed_image_ids.json'.format(generator.set_name), 'w'), indent=4)

    # load results in COCO evaluation tool
    coco_true = generator.coco
    coco_pred = coco_true.loadRes('6_tract_lesions.json')

    # run COCO evaluation
    coco_eval = COCOeval(coco_true, coco_pred, 'bbox')
    coco_eval.params.imgIds = image_ids
    coco_eval.params.catIds = [1,2,3,4,5,6]
    coco_eval.evaluate()
...
if __name__ == "__main__":
    generator=COCOGenerator("/data/test/",
                            "/data/COCO/annotations/test.json")
    model=FCOSDetector(mode="inference")
    # model = torch.nn.DataParallel(model, device_ids=[0, 1])
    model = model.cuda().eval()
    model.load_state_dict(torch.load("./checkpoint/6_tract_lesions/best_resnet50.pth",
