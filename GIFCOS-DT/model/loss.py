...
class GenTargets(nn.Module):
    ...
    def forward(self,inputs):
        ...
        for level in range(len(cls_logits)):
            ...
            level_targets=self._gen_level_targets(level_out,gt_boxes,classes,cnts_maps,self.strides[level],
    ...
    def _gen_level_targets(self,out,gt_boxes,classes,cnts_maps,stride,limit_range,sample_radiu_ratio=2):
        '''
        Args  
        ...
        b,c,h,w = cls_logits.shape          #TODO
        batch_size=cls_logits.shape[0]
        class_num=cls_logits.shape[1]
        m=gt_boxes.shape[1] 

        # Preparing the new center map
        cnts_maps = T.Resize(size=(h,w))(cnts_maps)             # TODO
        cnts_maps = torch.reshape(cnts_maps,(batch_size,-1,1))
        cnts_maps = cnts_maps / torch.max(cnts_maps).item()
        # cnts_maps[cnts_maps < 0.2] = -1
        ...
        assert cnts_maps.shape==(batch_size,h_mul_w,1)
        ...
        cnts_maps[~mask_pos_2]=-1
        reg_targets[~mask_pos_2]=-1
        
        
        return cls_targets,cnts_maps,reg_targets
